...
import (
	...
	"fmt"
	"log"
	"mitsuki/internal/dao/mysql"
	"mitsuki/internal/dao/redis"
	"mitsuki/internal/routers"
	"mitsuki/pkg/logger"
	"mitsuki/settings"
	"net/http"
	"os"
...
func main() {
	...
	// 1. 通过os.Args 执行命令行
	/* 	if len(os.Args) < 2 {
		fmt.Println("need set config file such: mitsuki.exe ./configs/config.yaml")
		return
	} */
	...
	var filepath string
	func() {
		flag.StringVar(&filepath, "f", "./configs/config.yaml", "配置文件")
		flag.Parse()
	}()
